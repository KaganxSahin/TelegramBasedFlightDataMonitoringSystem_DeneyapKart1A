...
Deneyap GPS ve GLONASS Konum Belirleyici (Quectel L86-M33 modülü ve STM8S003F3 mikroişlemcisi)
Deneyap Basınç Ölçer (TE Connectivity MS5637-02BA03)
Deneyap 6-Eksen Ataletsel Ölçüm Birimi (ST Microelectronics LSM6DSM)
Deneyap Ultraviyole Işık Algılayıcı (Lite-On LTR-390UV-01)
...
void loop() {
    ...
    // Barometrik İrtifa Denklemi (Barometric Altitude Equation)
    float altitude = 44330 * (1.0 - pow(pressure / 1013.25, 0.1903));

    String locationMessage = "Konum (Location): " + String(lat, 7) + ", " + String(lng, 7);
    telegram.send(CHAT_ID, locationMessage);

    String accelMessage = "İvme (Accel) (X,Y,Z): " + String(accelX) + ", " + String(accelY) + ", " + String(accelZ);
    telegram.send(CHAT_ID, accelMessage);

    String gyroMessage = "Jiro (Gyro) (X,Y,Z): " + String(gyroX) + ", " + String(gyroY) + ", " + String(gyroZ);
    telegram.send(CHAT_ID, gyroMessage);

    String tempMessage = "Sıcaklık (Temperature): " + String(tempC) + " °C, " + String(tempF) + " °F";
    ...
    String altitudeMessage = "İrtifa (Altitude): " + String(altitude) + " meters";
    telegram.send(CHAT_ID, altitudeMessage);

    String luxMessage = "Işık Şiddeti (LUX): " + String(lux);
